...
侧栏「TeX 文件结构」是独立的可选功能：选择已保存到书库的结果，在工程副本中由最终编译模型整理 `body.tex` 为章、节文件；原结果及书库当前版本不变。整理完成后，程序要求按 `body.tex` 的 `\input` 顺序重组出的正文与原工程逐字节一致、其他原工程文件不变，并重新运行 XeLaTeX；通过后在该 Tab 预览输出 PDF、下载分层工程，或手动「保存到书库」设为当前版本（旧版文件保留）。遇到含糊的标题边界或校验/编译失败，任务会保留副本并显示错误，可继续让 Agent 处理，不会自动更新书库。旧版、模板和修复产生的已保存工程也可以选择，前提是它们有 `body.tex`。
