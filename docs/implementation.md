...
更新：2026-09-07。当前代码只维护五步视觉转换路径，旧版数据兼容限制在历史列表和产物下载。

| 文件 | 职责 |
| --- | --- |
| `models.py` | 创建任务参数、状态和阶段定义 |
| `engine.py` | 五步调度、有限并行、接缝、断点恢复和编译修复 |
...
`.bookanalyst/bookanalyst.sqlite3` 存储书目、连接、运行摘要、任务和调用账本。正文不塞进运行记录：`.bookanalyst/runs/<id>/v6/` 下保存设置、批次、接缝、标题、最终页索引及 `tex/` 工程；原始请求在同级 `requests/` 下。
...
26 项程序测试及完整 Wang 45 页真实运行已经通过；具体严格读隔离、真实验证范围和语义保真度限制见 [工作流](workflow-v0.6.md)和 [验证记录](validation-v0.6.md)，不以程序测试代替真实证据。
