...
在“模型设置”中点击“添加 API 供应商”可连续添加任意多个 OpenAI 兼容供应商，每个供应商独立保存地址、密钥、协议、模型与并发设置。新供应商默认未启用；填写配置并启用后，即可在各阶段的“模型连接”中选择，最后点击“保存设置”。
...
实际请求同时受运行并发与连接并发约束。设置页改连接会影响之后新建的运行；进行中的运行要在运行页显式应用新绑定，失败不会自动切换渠道。
