...
const states={PENDING:"待开始",RUNNING:"运行中",PASSED:"已通过",NEEDS_REVIEW:"需要处理",FAILED:"未通过",STALE:"已失效",TEST_COMPLETED:"解析测试完成",SAMPLE_ACCEPTED:"样本已验收",BOOK_ACCEPTED:"全书已验收",FIXTURE_PASSED:"离线夹具通过"};
const profiles={cloud_smoke:"线上解析测试",local_smoke:"本地解析测试",offline_fixture:"离线程序夹具",llm_smoke:"样本转换",book:"书籍转换"};
const roles={analyst:"结构分析",converter:"TeX 转换",reviewer:"独立审查",visual_reviewer:"多模态核对"};
...
<div class="actions"><button class="button small" type="button" data-action="account-status">检查登录状态</button><button class="button small primary" type="button" data-action="login">登录 ChatGPT ↗</button></div><p id="account-status" class="status-detail"></p></section>
...
<label>连接并发上限<input name="custom_concurrency" type="number" min="1" value="${c.max_in_flight}"></label></div></section>
...
<div class="form-grid"><label>LLM 累计请求预算<input id="rerun-llm-budget" type="number" min="${r.config.max_llm_requests??0}" value="${r.config.max_llm_requests??""}" placeholder="不限"></label>
...
<section class="column"><div class="column-header">TeX 与数学表达<small>${d.tex?"已生成候选工程":"逐块转换"}</small></div><div class="column-content">${texNodes||empty("等待转换","结构与全书规则通过后生成 TeX。")}</div></section></div>
...
<section class="panel audit-panel"><h3>原图核对与差异证据</h3><div class="audit-row">${visual?`${visual.evidence_origin==="fixture"?"预录夹具证据 · ":""}完整页 ${visual.full_pages.length} / 计划 ${visual.planned_regions.length} · 局部区域 ${visual.crop_regions.length} · 未解决 ${visual.unresolved_regions.length}`:"尚未核对原图。解析成功不代表已完成视觉核对。"}</div>
...
if(button.dataset.confirmRerun){button.disabled=true;await api(`/api/runs/${runId}/rerun`,"POST",{revision:viewData.run.revision,operation_id:crypto.randomUUID(),stage:button.dataset.confirmRerun,task_id:button.dataset.task||null,force_recompute:$("#rerun-recompute").checked,
...
const fields=new FormData(form),payload={};for(const key of ["book_id","profile","parser","visual_mode"])payload[key]=fields.get(key);
for(const key of ["start_page","end_page","llm_concurrency","max_llm_requests","max_parse_submissions","max_submitted_pages"])payload[key]=Number(fields.get(key));
...
o.model_id=f.get("official_model");o.max_in_flight=Number(f.get("official_concurrency"));
Object.assign(c,{enabled:f.has("custom_enabled"),base_url:f.get("custom_url"),protocol:f.get("custom_protocol"),model_id:f.get("custom_model"),auth_mode:f.get("custom_auth"),api_key_env:f.get("custom_env"),image_support:f.get("image_support"),max_in_flight:Number(f.get("custom_concurrency"))});
