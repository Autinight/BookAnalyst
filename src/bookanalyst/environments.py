            ...
            for match in TOKEN.finditer(line):
                ...
                if operation == "label":
                    if stack and not stack[-1]["label"]:
                        ...
                        if kind in ("align", "alignat", "gather", "multline", "eqnarray", "flalign"):
                            kind = "equation"
                ...
                if operation == "begin":
                    ...
                    if match_index is not None:
                        stack.pop(match_index)["end"] = location
                    else:
                        end_ordinal += 1
                        unmatched.append({"id": f"end-{page['page']}-{end_ordinal}", "name": name, **location})
