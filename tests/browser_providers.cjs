...
(async () => {
  ...
  try {
    ...
    const ids = [];
    for (let i = 0; i < 3; i++) {
      await page.getByRole('button', { name: '添加 API 供应商', exact: true }).click();
      ...
      await card.getByLabel('模型', { exact: true }).fill(`model-${i}`);
      await card.locator('[data-connection-field="api_key"]').fill(`test-provider-key-${i}`);
      await card.getByLabel('启用', { exact: true }).check();
      assert.equal(await card.getByRole('button', { name: '检查连接' }).isDisabled(), true);
    }
    ...
    const save = async () => {
      ...
      assert.equal((await response.text()).includes('test-provider-key'), false);
      await page.waitForFunction(() => [...document.querySelectorAll('[data-connection-field="api_key"]')].every(input => input.value === ''));
      return response.json();
    };
    let saved = await save();
    assert.equal(Object.keys(saved.connections).length, 5);
    ...
    assert.equal(await first.locator('[data-connection-field="api_key"]').inputValue(), '');
    await page.route(`**/api/connections/${ids[1]}/test`, route => route.fulfill({ json: { status: 'READY', models: [{ id: 'probe-model' }] } }));
    ...
    await second.locator('[data-connection-field="clear_api_key"]').check();
    saved = await save();
